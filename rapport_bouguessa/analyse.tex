...
0,0068, NormTuning = LoRA au millième près avec $\approx$47k paramètres) ;
...
0,0068}. Aucun rang, aucun $\alpha$, aucun type ne bat l'ancre r8a8 tous blocs
...
% Les 108 rejets BH sont répartis en DEUX tableaux lisibles (générateur
% make_tables.t_signif_bh) : 1/2 les paires gagnées par les modèles de
% contexte (tab:signif), 2/2 celles gagnées sur tuile seule (tab:signiftile).
% Chaque gagnant n'est nommé qu'une fois, en tête de groupe ; les noms sont
% ceux du tableau~\ref{tab:ci} (backbone --- régime, état).
...
internes). Chaque axe : \emph{rang} du classement (celui du tableau~\ref{tab:ci},
où chaque nom court est déployé en entier --- backbone, régime, état) · nom
\emph{court} · F1 (en pourcentage). La couleur porte le $\Delta$ (rouge = la
ligne bat la colonne) ; $\bigstar$ = paire rejetée par Benjamini-Hochberg. La
première ligne et la première colonne sont D3-B~ctx~R2, qui bat toute la tête
(16~$\bigstar$ sur 16).}
\label{fig:signif_head}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{figs/perf_signif_matrix_tail.png}
\caption{Matrice de significativité (b) — queue du palier (16~modèles, mêmes
rangs que ci-dessus). Le gros bloc central sans $\bigstar$ est le palier plat
SimDINOv2-B : les bras d'adaptation ne se distinguent pas l'un de l'autre. Les
rares $\bigstar$ de ce bloc sont les victoires sur les bras effondrés : l'ancre
SimB (rang 18) bat SimB-MHSA (31), D3-S gelé (32) et IN-MHSA (33) ; D3-S LoRA
(19) et SimB LoRA~r16 (21) ne battent plus que D3-S gelé (32). Les $\Delta$
exacts sont dans les tableaux~\ref{tab:signif} et~\ref{tab:signiftile}.}
\label{fig:signif_tail}
\end{figure}
