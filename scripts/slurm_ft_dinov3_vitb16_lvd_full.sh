#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# FT SoTA — DINOv3 ViT-B/16 (LVD-1689M), régime FULL, 100% des données, 3 seeds.
# Miroir exact de scripts/slurm_sota_screening.sh (array 0, config vitb16_fulft_sota.yaml)
# mais pour le backbone DINOv3 — objectif : mesurer si le fine-tuning complet d'un SSL déjà
# fort (LVD) apporte un gain mesurable par rapport au frozen (dinov3_vitb16_lvd, déjà
# benchmarké), ou sature déjà en frozen.
#
# OUT_DIR dédié (sota_screening/dinov3_vitb16_lvd_full/), DISTINCT de sota_screening/full/
...
#
# Soumission (MANUELLE — MFA) :  sbatch scripts/slurm_ft_dinov3_vitb16_lvd_full.sh
# ═══════════════════════════════════════════════════════════════════════════════
#SBATCH --job-name=ft_dinov3b_full
#SBATCH --gres=gpu:a100_3g.20gb:1
#SBATCH --mem=40G
#SBATCH --cpus-per-task=4
#SBATCH --time=10:00:00
#SBATCH --output=logs/ft_dinov3b_full_%j.out
#SBATCH --error=logs/ft_dinov3b_full_%j.err
#SBATCH --account=def-bouguess_gpu

# ─── Configuration ─────────────────────────────────────────────────────────────
CONFIG="configs/ft_dinov3_vitb16_lvd_full.yaml"
REGIME_TAG="dinov3_vitb16_lvd_full"
...
# Modèles HuggingFace pré-téléchargés (login node) — pas de download sur nœud calcul.
# DINOv3-HF requiert transformers>=4.56 ; le cache doit déjà contenir
# facebook/dinov3-vitb16-pretrain-lvd1689m (même modèle que l'extraction frozen existante,
# configs/frozen_dinov3_lvd.yaml).
...
echo "═══════════════════════════════════════════════"
echo "FT DINOv3 ViT-B/16 LVD — régime=full  config=$CONFIG"
...
for SEED in 0 1 2; do
    echo ""
    echo "─── DINOv3-B/16 LVD  Régime=full  Fraction=$FRAC  Seed=$SEED ───"
    ...
    if [[ $EXIT_CODE -ne 0 ]]; then
        echo "[ERROR] run dinov3_vitb16_lvd/full seed=$SEED a échoué (exit=$EXIT_CODE)" >&2
...
echo ""
echo "═══════════════════════════════════════════════"
echo "[résumé] DINOv3-B/16 LVD — Régime=full — f1_macro_pres_test par seed"
echo "═══════════════════════════════════════════════"
...
if vals:
    ...
    print(f"  → [dinov3_vitb16_lvd/full] f1_macro_pres_test = {mean:.4f} ± {sd:.4f}  (n={len(vals)} seeds)")
else:
    print("  → [dinov3_vitb16_lvd/full] aucun run complété.")
PY

echo ""
echo "[slurm] Job $SLURM_JOB_ID (dinov3_vitb16_lvd/full) terminé."
