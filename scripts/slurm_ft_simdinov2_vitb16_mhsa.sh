#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# FT SoTA — SimDINOv2 ViT-B/16 (iNat21 Plantae), régime MHSA-only, 100% des données, 3 seeds.
# Miroir exact de scripts/slurm_sota_screening.sh (array 1, config vitb16_mhsa_sota.yaml)
# mais pour le backbone SimDINOv2.
#
# OUT_DIR dédié (sota_screening/simdinov2_vitb16_mhsa/), DISTINCT de sota_screening/mhsa/
...
#
# Soumission (MANUELLE — MFA) :  sbatch scripts/slurm_ft_simdinov2_vitb16_mhsa.sh
# ═══════════════════════════════════════════════════════════════════════════════
#SBATCH --job-name=ft_simdv2b_mhsa
#SBATCH --gres=gpu:a100_3g.20gb:1
#SBATCH --mem=40G
#SBATCH --cpus-per-task=4
#SBATCH --time=10:00:00
#SBATCH --output=logs/ft_simdv2b_mhsa_%j.out
#SBATCH --error=logs/ft_simdv2b_mhsa_%j.err
#SBATCH --account=def-bouguess_gpu

# ─── Configuration ─────────────────────────────────────────────────────────────
CONFIG="configs/ft_simdinov2_vitb16_mhsa.yaml"
REGIME_TAG="simdinov2_vitb16_mhsa"
...
echo "═══════════════════════════════════════════════"
echo "FT SimDINOv2 ViT-B/16 (iNat Plantae) — régime=mhsa  config=$CONFIG"
...
for SEED in 0 1 2; do
    echo ""
    echo "─── SimDINOv2-B/16  Régime=mhsa  Fraction=$FRAC  Seed=$SEED ───"
    ...
    if [[ $EXIT_CODE -ne 0 ]]; then
        echo "[ERROR] run simdinov2_vitb16/mhsa seed=$SEED a échoué (exit=$EXIT_CODE)" >&2
...
echo ""
echo "═══════════════════════════════════════════════"
echo "[résumé] SimDINOv2-B/16 — Régime=mhsa — f1_macro_pres_test par seed"
echo "═══════════════════════════════════════════════"
...
if vals:
    ...
    print(f"  → [simdinov2_vitb16/mhsa] f1_macro_pres_test = {mean:.4f} ± {sd:.4f}  (n={len(vals)} seeds)")
else:
    print("  → [simdinov2_vitb16/mhsa] aucun run complété.")
PY

echo ""
echo "[slurm] Job $SLURM_JOB_ID (simdinov2_vitb16/mhsa) terminé."
